...
public class NewCombinationStrategy implements IStrategy {
	...
	@Override
	public void operate(List<LotteryModel> lotteryList,
			StorageModel<QueryResult> storage, char[][] charArrays,
			String left, String right, int stage) {
		String[] currentArrays = convertChar(charArrays);
		int size = lotteryList.size();
		try {
			for (int i = 0; i < size; i++) {
				if (i + stage < size) {
					splitContrast(lotteryList, storage, i, i + stage,
							currentArrays, left, right);
				}
			}
	...
	private void splitContrast(List<LotteryModel> lotteryList,
			StorageModel<QueryResult> storage, int index, int stage,
			String[] currentArrays, String left, String right)
		...
		for (int i = index; i < stage; i++) {
			...
			boolean currentFlag = DigitUtil.isContain(3, left,
					currentArrays[j++]);
			boolean codeFlag = DigitUtil.isContain(3, left, code);
		...
		storage.push(new QueryResult(lotteryList.get(index).getNumber(), left,
				right, 0, stage - index));
	}
}
