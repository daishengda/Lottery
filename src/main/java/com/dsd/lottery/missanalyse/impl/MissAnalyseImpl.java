...
public class MissAnalyseImpl implements IMissAnalyse {
	...
	public List<MissInfoModel> queryMissInfo(Map<String, String> param) {
		List<MissGroupModel> missGroupList = myBatisDAO.selectList(
				"LotteryMissMapper.queryMissGroup", param);
		...
		for (MissRelationModel missRelation : missRelationList) {
			buildMap(groupIdMap, missRelation.getGroupId(),
					missRelation.getResultId());
			resultIds.add(missRelation.getResultId());
		}
		paramObj.clear();
		paramObj.put("ids", resultIds);
		List<MissResultModel> missResultList = myBatisDAO.selectList(
				"LotteryMissMapper.queryMissResult", paramObj);
		...
		for (MissGroupModel missGroup : missGroupList) {
			...
			missInfo.setDigit(missGroup.getDigit());
			...
			for (Long resultId : ids) {
				...
				missInfo.getMissList()
						.add(new MissModel(result.getBegin(), result.getEnd(),
								result.getType(), desc, result.getMissPeriods()));
			}
			// 排序
			Collections.sort(missInfo.getMissList(),
					new Comparator<MissModel>() {
						@Override
						public int compare(MissModel o1, MissModel o2) {
							int diffType = o1.getType() - o2.getType();
							return diffType == 0 ? (int) (o1.getEnd() - o2
									.getEnd()) : diffType;
						}
					});
