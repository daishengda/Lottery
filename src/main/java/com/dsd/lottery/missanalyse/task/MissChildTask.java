...
public class MissChildTask implements Runnable {
    ...
    public MissChildTask(List<LotteryModel> lotteryList, MissGroupModel missGroup,
            AtomicLong atomic, MyBatisDAO myBatisDAO) {
    ...
    public void run() {
        /*
         * LogUtil.info(String.format( "start calculate MissChildTask!digit = %d,group = %s ",
        ...
        EnumMissGroup enumMiss = EnumMissGroup.getEnumMissGroup(missGroup.getDigit());
        for (int type : enumMiss.getMatch()) {
            buildMissResult(missResultList, relationLst, type);
        }
        /*
         * LogUtil.info(String.format( "end calculate MissChildTask!digit = %d,group = %s ",
         * missGroup.getDigit(), missGroup.getGroup()));
         */
    ...
    private void buildMissResult(List<MissResultModel> missResultList,
        ...
        missResultList.add(new MissResultModel(currentId, longArrays[2], 0L, intArrays[1], type,
                digit));
        missResultList.add(new MissResultModel(maxId, longArrays[0], longArrays[1], intArrays[0],
                type, digit));
    ...
    private int[] buildIntArrays(int type) {
        String key =
                ResourceUtil.format(MissComparatorFactory.KEY_FORMAT, missGroup.getDigit(), type);
        IMissComparator missComp = MissComparatorFactory.getMissComparator(key);
        int size = lotteryList.size();
        // [0] 最大遗漏值,[1]往期当前遗漏值,[2]最大遗漏值出现的索引,[3]往期遗漏值出现的索引
        int[] intArrays = new int[4];
        for (int i = 0; i < size; i++) {
            boolean isEqual =
                    missComp.equals(type, missGroup.getGroup(), lotteryList.get(i).getCode());
            if (isEqual) {
    ...
    private long[] buildLongArrays(int[] intArrays) {
        ...
        if (intArrays[1] >= intArrays[0]) {
            ...
            longArrays[0] = lotteryList.get(intArrays[2] - intArrays[0]).getSortid();
    ...
    protected void afterHandle(List<MissResultModel> missResultList,
        ...
        myBatisDAO.batchHightInsert("LotteryMissMapper.insertBatchMissResult", missResultList);
        myBatisDAO.batchHightInsert("LotteryMissMapper.insertBatchMissRelation", relationLst);
    }
